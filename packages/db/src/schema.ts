...
export const artifactVersions = pgTable(
  ...
  (table) => ({
    artifactHashUnique: uniqueIndex('artifact_versions_artifact_hash_unique').on(
      table.artifactId,
      table.contentHash,
    ),
  }),
...
export const sessions = pgTable(
  ...
  (table) => ({
    employeeStartedIdx: index('sessions_employee_started_idx').on(
      table.employeeId,
      table.startedAt,
    ),
    artifactVersionUnique: uniqueIndex('sessions_artifact_version_unique').on(
...
export const messages = pgTable(
  ...
  (table) => ({
    sessionOrdinalUnique: uniqueIndex('messages_session_ordinal_unique').on(
      table.sessionId,
      table.ordinal,
    ),
  }),
...
export const modelPrices = pgTable(
  'model_prices',
  {
    ...
    cachedInputUsdPerMillion: numeric('cached_input_usd_per_million', { mode: 'number' })
      .notNull()
      .default(0),
    cacheCreationUsdPerMillion: numeric('cache_creation_usd_per_million', { mode: 'number' })
      .notNull()
      .default(0),
    reasoningOutputUsdPerMillion: numeric('reasoning_output_usd_per_million', { mode: 'number' })
      .notNull()
      .default(0),
