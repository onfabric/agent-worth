...
describe("source adapters", () => {
  test("scans synthetic Codex JSONL and produces CloudEvents", async () => {
    ...
    await Bun.write(
      path,
      [
        JSON.stringify({ type: "session_meta", payload: { id: "session-1", model_provider: "openai" } }),
        JSON.stringify({ type: "response_item", payload: { role: "user", content: "synthetic prompt" } })
      ].join("\n")
    );
